/**
 * Prime Video 解锁检测
 * 检测当前节点能否访问 Prime Video，并识别可用区域
 */
    ...
    return new Promise((resolve) => {
        $httpClient.get(
            ...
            (error, response, data) => {
                ...
                if (status === 403 || data.includes('not available in your region')) {
                    resolve('不可用');
                    return;
                }

                // 提取地区
                const regionMatch = data.match(/"currentTerritory"\s*:\s*"([A-Z]{2})"/)
                    || data.match(/"marketplaceId"\s*:\s*"([A-Z]{2})"/)
